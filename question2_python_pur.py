...
# Fonction pour convertir une date en tuple (année, mois, jour) pour comparaison
...
game_reg_season_22_23 = [
    ...
    if debut_saison_22_23 <= game['game_date'] <= fin_saison_22_23 and game['season_type'] == 'Regular Season'
]

# Compter le nombre total de matchs joués par chaque équipe
total_games = {}
for game in game_reg_season_22_23:
    total_games[game['team_abbreviation_home']] = total_games.get(game['team_abbreviation_home'], 0) + 1
    total_games[game['team_abbreviation_away']] = total_games.get(game['team_abbreviation_away'], 0) + 1

# Compter le nombre de victoires par équipe
wins = {}
for game in game_reg_season_22_23:
    winner = game['team_abbreviation_home'] if game['wl_home'] == 'W' else game['team_abbreviation_away']
...
ratio_with_conference = [{'abbreviation': team, 'Win Ratio': ratio[team], 'conf': teams_conferences[team]} for team in ratio]

# Fonction pour trier les équipes par ratio de victoires décroissant
def sort_by_win_ratio(teams):
    return sorted(teams, key=lambda x: x['Win Ratio'], reverse=True)

# Trier les équipes par ratio de victoires décroissant
ratio_with_conference = sort_by_win_ratio(ratio_with_conference)
...
for team in classement_west:
    print(f"{team['classement']}: {team['abbreviation']} - Win Ratio: {team['Win Ratio']:.4f}")
