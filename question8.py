...
# et dont le nom de l'organisation contient des parenthèses, mais pas "G League"
draft_stranger = draft[
    (draft['organization_type'] == 'Other Team/Club') &
    (draft['organization'].str.contains(r'\(')) &  # Utiliser r'\(' pour échapper la parenthèse
    (~draft['organization'].str.contains('IBL'))&
    (~draft['organization'].str.contains('G League'))# Filtrer les organisations contenant "G League"
]
...
plt.tight_layout()  # Ajustement automatique des marges pour éviter les chevauchements
plt.show()
