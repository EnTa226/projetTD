...
def run_question_2():
    ...
    total_games = (
        game_reg_season_22_23['team_abbreviation_home'].value_counts() +
    ...
    output += classement_west[['classement', 'abbreviation', 'Win Ratio',
                               'Avg Points']].to_string(index=False)
    output += "\n\nClassement Conférence Est :\n"
    output += classement_east[
        ['classement', 'abbreviation', 'Win Ratio',
         'Avg Points']].to_string(index=False)
...
def run_question_2bis():
    def read_csv(file_path, delimiter=','):
        with open(file_path, 'r') as f:
            lines = f.readlines()
        headers = lines[0].strip().split(delimiter)
        return [dict(zip(headers, line.strip().split(delimiter)))
                for line in lines[1:]]
    ...
    season_games = [g for g in game_data if debut <= g['game_date'] <= fin
    ...
    for team in wins:
        ...
        teams.append({'abbr': team, 'ratio': ratio,
                      'avg_pts': avg_pts, 'conf': confs[team]})

    teams.sort(key=lambda x: (x['ratio'], x['avg_pts']), reverse=True)

    output = ""
    rank = 1
    output += "Clasement Conférence Ouest :\n"
    for t in [t for t in teams if t['conf'] == 'W']:
        output += (
            f"{rank}. {t['abbr']} - Ratio: {t['ratio']:.3f}, "
            f"Moy. points: {t['avg_pts']:.2f}\n"
        )
        rank += 1
...
def run_question_3():
    ...
    output = (
        f"L'équipe avec le meilleur ratio de victoire est :\n"
        f"{best_team_pandas}"
        f"({best_ratio_pandas:.3f})\n\n"
        f"Dans le cas où les équipes ont fait au moins 30 matchs\n"
        f"La meilleure équipe est {best_team_filtered}"
        f"({best_ratio_filtered:.3f})"
    )
...
def run_question_3_bis():
    ...
    return (
        f"L'équipe avec le meilleur ratio de victoire est : {best_team} "
        f"({best_ratio:.2%})"
    )
...
def run_question_4(annee):
    ...
    try:
        ...
        home_fg3_pct = game_year.groupby(
            'team_abbreviation_home')['fg3_pct_home'].sum()
        away_fg3_pct = game_year.groupby(
            'team_abbreviation_away')['fg3_pct_away'].sum()
        total_fg3_pct = home_fg3_pct.add(away_fg3_pct, fill_value=0)
        ...
        return (f"L'équipe avec en moyenne le plus de réussite aux "
...
def mean_height_by_position(players, draft_target):
    ...
    for p in players:
        if (
            p["draft_number"] == draft_target
            and p["height_cm"] is not None
        ):
            pos = p["position"]
            position_totals[pos] = position_totals.get(pos, 0) + \
                                    p["height_cm"]
...
def run_question_6_bis():
    ...
    def mean_height_by_position(players, draft_target):
        ...
        for p in players:
            if (
                p["draft_number"] == draft_target and
                p["height_cm"] is not None
            ):
                pos = p["position"]
                position_totals[pos] = (
                    position_totals.get(pos, 0) + p["height_cm"]
                )
                position_counts[pos] = position_counts.get(pos, 0) + 1
    ...
    try:
        ...
        if df_clean.empty:
            return (
                ...
                "(vérifiez la colonne 'height' ou 'position')."
            )
...
def run_question_8():
    ...
    annual_counts = draft.groupby(
        'season').size().reset_index(name='count_total')
    ...
    strangers_annual_counts = draft_stranger.groupby(
        'season').size().reset_index(name='count_stranger')

    merged_df = pd.merge(
        annual_counts, strangers_annual_counts, on='season', how='left')
    merged_df['count_stranger'] = merged_df['count_stranger'].fillna(0)
    merged_df['ratio'] = (
        merged_df['count_stranger'] / merged_df['count_total']) * 100
...
def run_question_9():
    ...
    print("Le numéro de maillot le plus utilisé par les joueurs est")
...
def open_window(titre, fonction, code, besoin_annee=False):
    ...
    tk.Label(win, text="Code de la question :",
...
def main_menu():
    ...
    tk.Button(root, text="Question 1",
              font=("Helvetica", 13), width=30,
              command=lambda: open_window("Question 1",
                                          run_question_1, code_question_1)
              ).pack(pady=5)

    tk.Button(root, text="Question 2 pandas", font=("Helvetica", 13), width=30,
              command=lambda: open_window(
                  "Question 2", run_question_2, code_question_2)).pack(pady=5)

    tk.Button(root, text="Question 2 python pur",
              font=("Helvetica", 13),
              width=30,
              command=lambda: open_window(
                  "Question 2 bis - Classement 2022-2023",
                  run_question_2bis, code_question_2bis)).pack(pady=5)
    ...
    tk.Button(root,
              ...
              width=30,
              command=lambda: open_window(
                  "Question 3 - Meilleur ratio victoire", run_question_3,
                  code_question_3_bis)).pack(pady=5)

    tk.Button(root, text="Question 3 python pur", font=("Helvetica", 13),
              width=30,
              command=lambda: open_window("Question 3 - NBA",
                                          run_question_3_bis, code_q3_pur)
              ).pack(pady=3)

    tk.Button(root, text="Question 4", font=("Helvetica", 13), width=30,
              command=lambda: open_window("Question 4", run_question_4,
                                          code_q4, besoin_annee=True)
              ).pack(pady=5)

    tk.Button(root, text="Question 5", font=("Helvetica", 13), width=30,
              command=lambda: open_window("Question 5", run_question_5,
                                          code_question_5, besoin_annee=True)
              ).pack(pady=5)

    tk.Button(root, text="Question 6 - pandas", font=("Helvetica", 13),
              width=30,
              command=lambda: open_window(
                  "Question 6", run_question_6, code_q6)).pack(pady=3)

    tk.Button(root, text="Question 6 - python pur", font=("Helvetica", 13),
              width=30,
              command=lambda: open_window(
                  "Question 6", run_question_6_bis, code_q6_bis)).pack(pady=3)

    tk.Button(root, text="Question 7", font=("Helvetica", 13), width=30,
              command=lambda: open_window(
                  "Question 7", run_question_7, code_question_7)).pack(pady=5)

    tk.Button(root, text="Question 8", font=("Helvetica", 13), width=30,
              command=lambda: open_window(
                  "Question 8", run_question_8, code_question_8)).pack(pady=5)

    tk.Button(root, text="Question 9", font=("Helvetica", 13), width=30,
              command=lambda: open_window(
                  "Question 9", run_question_9, code_question_9)).pack(pady=5)

    tk.Button(root, text="Question 10 - Pandas", font=("Helvetica", 13),
              width=30,
              command=lambda: open_window(
                  "Question 10 - Pandas", run_question_10_pandas,
                  code_q10_pandas)).pack(pady=3)

    tk.Button(root, text="Question 10 - Python pur", font=("Helvetica", 13),
              width=30,
              command=lambda: open_window(
                  "Question 10 - Python pur", run_question_10_pure,
                  code_q10_pure)).pack(pady=3)
